...
ExceptionHandler(ExceptionType which)
{
    ...
    else if(which == PageFaultException)
    {
    	printf("PageFaultException\n");
         ...
         int badAddr = machine -> ReadRegister(39);
         int vpn = (unsigned) badAddr / PageSize;
        int x;
        if(memoryManager->IsAnyPageFree() == true)
            	x = memoryManager->AllocPage(currentThread->id,machine->pageTable[vpn]);
        else
        {
           x = memoryManager->AllocByForce();
        }
          currentThread->space->loadIntoFreePage(badAddr, x);
